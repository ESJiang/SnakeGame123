...
def load_image(filename):
    return pygame.image.load(os.path.join(package_base_path, "Assets", "Graphics", filename)).convert_alpha()


def load_sound(filename):
    return pygame.mixer.Sound(os.path.join(package_base_path, "Assets", "Sound", filename))
...
class Fruit:
    ...
    def random_place(self):
        self.pos = Vector2(random.choice(range(CELL_NUMBER)), random.choice(range(CELL_NUMBER)))
...
class SnakeGame:
    ...
    def __game_over(self):
        ...
        gameover_location = gameover_color.get_rect(midtop=(int(CELL_SIZE * CELL_NUMBER / 2), 20))
    ...
    def welcome(msg):
        ...
        while True:
            clock.tick(50)  # avoid the program from using too much CPU resources (30~60)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    SnakeGame.__close_game()
                elif event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and text_rect.collidepoint(event.pos):
                    return
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_UP and snake_game.snake.direction != Vector2(0, 1):
                        snake_game.snake.direction = Vector2(0, -1)
                    elif event.key == pygame.K_DOWN and snake_game.snake.direction != Vector2(0, -1):
                        snake_game.snake.direction = Vector2(0, 1)
                    elif (
                    ...
                    elif event.key == pygame.K_RIGHT and snake_game.snake.direction != Vector2(-1, 0):
                        snake_game.snake.direction = Vector2(1, 0)
